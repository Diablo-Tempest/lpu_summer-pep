...
3. Doesn't support seeking efficiently

With Streams
1. very little memory usage
2. Faster
3. Supports buffering and seeking
...
// fetch filename and dirname

//import.meta stores metadata about the current module.
...
const server = createServer(((req, res) => {
    ...
    if (req.url === '/') {
        ...
        //C:Trainings/LPU/Session1/day14/public/index.html
        createReadStream(htmlPath).pipe(res);
        return;
        // createReadStream:
        //creates a readable stream for the file.
        // reads the file in small chunks rather than loading the file into the memory.
    ...
    if (req.url === '/video') {
        ...
        // gets the range header sent by the browser
        ...
        const CHUNK_SIZE = 10 ** 6 // 1MB 1,000,000 bytes 
        const start = Number(range.replace(/\D/g, "")); // bytes = 2000- = 2000
        const end = Math.min(start + CHUNK_SIZE, fileSize - 1);
        //ensures that ending byte never goes past the end opf the file.
        const contentLength = end - start + 1;
        res.writeHead(206, {
            "Content-Range": `bytes ${start}-${end}/${fileSize}`,
            //0 - 999999/100000000
            "Accept-Ranges": "bytes", // tells the browser that this server supports byte-renge request
